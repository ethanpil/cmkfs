...
func TestBuildGolden(t *testing.T) {
	cases := []struct {
		...
		{
			name:        "vfat all defaults",
			schemaID:    "vfat",
			device:      "/dev/loop9",
			wantArgv:    []string{"mkfs.fat", "/dev/loop9"},
			wantDisplay: "mkfs.fat /dev/loop9",
		},
		{
			name:     "vfat label fat size and volume id",
			schemaID: "vfat",
			values: map[string]any{
				"label":     "EFI",
				"fat_size":  "32",
			...
			device:      "/dev/sdb1",
			wantArgv:    []string{"mkfs.fat", "-n", "EFI", "-F", "32", "-i", "DEADBEEF", "/dev/sdb1"},
		...
		},
		{
			...
			device:      "/dev/sdb",
			wholeDisk:   true,
			wantArgv:    []string{"mkfs.fat", "-I", "/dev/sdb"},
			wantDisplay: "mkfs.fat -I /dev/sdb",
		},
		{
			name:        "vfat force is a no-op (no signature gate)",
			schemaID:    "vfat",
			device:      "/dev/sdb1",
			force:       true,
			wantArgv:    []string{"mkfs.fat", "/dev/sdb1"},
		...
		{
			name:        "f2fs all defaults",
			schemaID:    "f2fs",
			device:      "/dev/loop9",
			wantArgv:    []string{"mkfs.f2fs", "/dev/loop9"},
			wantDisplay: "mkfs.f2fs /dev/loop9",
		},
		{
			name:        "f2fs label",
			schemaID:    "f2fs",
			values:      map[string]any{"label": "android"},
			device:      "/dev/mmcblk0p3",
			wantArgv:    []string{"mkfs.f2fs", "-l", "android", "/dev/mmcblk0p3"},
			wantDisplay: "mkfs.f2fs -l android /dev/mmcblk0p3",
		},
		{
			name:        "f2fs force injection position",
			schemaID:    "f2fs",
			device:      "/dev/sdb1",
			force:       true,
			wantArgv:    []string{"mkfs.f2fs", "-f", "/dev/sdb1"},
			wantDisplay: "mkfs.f2fs -f /dev/sdb1",
		},
		{
